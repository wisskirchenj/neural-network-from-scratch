...
def load_train_and_test_data() -> tuple[DataFrame, DataFrame]:
    if not os.path.exists(PATH_DATA):
        print('Create Data Directory ...')
        os.mkdir(PATH_DATA)
    # Download data if it is unavailable.
    files = os.listdir(PATH_DATA)
    if not all(csv in files for csv in (TEST_CSV, TRAIN_CSV)):
        download_data()
    return read_csv_as_bytes(TRAIN_CSV), read_csv_as_bytes(TEST_CSV)


def read_csv_as_bytes(filename: str) -> DataFrame:
    # as data describe pixel intensities in the range of 0..255, it is efficient to load them as unsigned bytes
    return read_csv(PATH_DATA + '/' + filename, dtype=defaultdict(lambda: uint8))
